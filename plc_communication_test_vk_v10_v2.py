...
def update_PLC_Status_UI(STATUS): # NOT
    db_update = None
    cur = None
    try:
...
class ImageCapture:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.error("Error starting initCam thread: %s", traceback.format_exc())
    ...
    def acquire_images(self, cam_list):
        try:
            for cam in cam_list:
                ...
                node_acquisition_mode = PySpin.CEnumerationPtr(cam.GetNodeMap().GetNode('AcquisitionMode'))
                if not PySpin.IsAvailable(node_acquisition_mode) or not PySpin.IsWritable(node_acquisition_mode):
                    logger.error("Acquisition mode not available or writable.")
                    return False

                node_acquisition_mode_continuous = node_acquisition_mode.GetEntryByName('Continuous')
                if not PySpin.IsAvailable(node_acquisition_mode_continuous) or not PySpin.IsReadable(
                        node_acquisition_mode_continuous):
                    logger.error("Continuous acquisition mode not available or readable.")
                    return False
            ...
            while not self.stopThread:
                try:
                    ...
                    if actNotReached is True:
                        ...
                        plcCommunicationObj.writeIntToPLC(clientConn,6,0)
                        logger.debug(f"================engine number is {Engine_No} Position {Position} actNotReached is True:")
                                    ...
                                    if len(capture_list) >=8:
                                        while True:
                                           ...
                                           # IS_PROCESS, OverallOk, CamCapstatus, HLA_status = CamCap_Sealant_result_status()
                                            ...
                                            HLA_status = "OK"
                                            IS_PROCESS = 2
                                            logger.debug(f"3cylinder Engine Number is :{Engine_No} and IS_PROCESS {IS_PROCESS} OverallOk {OverallOk} HLA_status {HLA_status} and CamCapstatus {CamCapstatus} ")
                                            if IS_PROCESS == 2 and CamCapstatus =="OK" and HLA_status == "OK":
                                                imageounter = 0
                                                CAPTURE_FLAG = True
                                            ...
                                            elif IS_PROCESS == 2 and CamCapstatus =="NOT OK" and HLA_status =="NOT OK":
                                                CAPTURE_FLAG = True
                                                # plcCommunicationObj.writeIntToPLC(clientConn,0,1)#overallok
                                                # plcCommunicationObj.writeIntToPLC(clientConn,10,1)#overallokcamcap
                                                # plcCommunicationObj.writeIntToPLC(clientConn,8,1)#overallokcamcaphla
                                            ...
                                            #  time.sleep(5)
                                            ...
                                            elif IS_PROCESS == 2 and CamCapstatus =="NOT OK" and HLA_status =="OK":
                                                CAPTURE_FLAG = True
                                                # plcCommunicationObj.writeIntToPLC(clientConn,0,1)#overallok
                                                # plcCommunicationObj.writeIntToPLC(clientConn,10,1)#overallokcamcap
                                                # plcCommunicationObj.writeIntToPLC(clientConn,8,1)#overallokcamcaphla
                                                ...
                                                logger.debug(f"4cylinder Engine Number is :{Engine_No} and CamCap overall not ok but send ok  Status sent to plc")
                                            ...
                                            elif IS_PROCESS == 2 and CamCapstatus =="OK" and HLA_status =="NOT OK":
                                                CAPTURE_FLAG = True
                                                # plcCommunicationObj.writeIntToPLC(clientConn,0,1)#overallok
                                                # plcCommunicationObj.writeIntToPLC(clientConn,10,1)#overallokcamcap
                                                # plcCommunicationObj.writeIntToPLC(clientConn,8,1)#overallokcamcaphla
                                                ...
                                                logger.debug(f"4cylinder Engine Number is :{Engine_No} and hla overall not ok  send c")
                                ...
                                if image_result.IsIncomplete():
                                    ...
                                    if len(capture_list) >=6:
                                        while True:
                                            ...
                                            print("in while capture list is ",len(capture_list))
                                            logger.debug(f"3cylinder Engine Number is :{Engine_No} and IS_PROCESS {IS_PROCESS} OverallOk {OverallOk} HLA_status {HLA_status} and CamCapstatus {CamCapstatus} ")
                                            if IS_PROCESS == 2 and HLA_status =="OK" and CamCapstatus =="OK":
                                                imageounter = 0
                                                CAPTURE_FLAG = True
                                                plcCommunicationObj.writeIntToPLC(clientConn,0,1)#overallok
                                                plcCommunicationObj.writeIntToPLC(clientConn,10,1)#overallokcamcap
                                                plcCommunicationObj.writeIntToPLC(clientConn,8,1)#overallok_hla
                                                
                                                logger.debug(f"3cylinder Engine Number is :{Engine_No} an HLA_status and CamCapstatus ok Status sent to plc")
                                            ...
                                            elif IS_PROCESS == 2 and HLA_status =="NOT OK" and CamCapstatus =="NOT OK":
                                                CAPTURE_FLAG = True
                                                plcCommunicationObj.writeIntToPLC(clientConn,0,2)#overallok
                                                plcCommunicationObj.writeIntToPLC(clientConn,10,1)#overallokcamcap
                                                plcCommunicationObj.writeIntToPLC(clientConn,8,2)#overallok_hla
                                         
                                                logger.debug(f"3cylinder Engine Number is :{Engine_No} and CamCap and HLA_status not ok Status sent to plc")
                                            ...
                                            elif IS_PROCESS == 2 and HLA_status =="NOT OK" and CamCapstatus =="OK":
                                                CAPTURE_FLAG = True
                                                plcCommunicationObj.writeIntToPLC(clientConn,0,2)#overallok
                                                plcCommunicationObj.writeIntToPLC(clientConn,10,1)#overallokcamcap
                                                plcCommunicationObj.writeIntToPLC(clientConn,8,2)#overallok_hla
                                             
                                                logger.debug(f"3cylinder Engine Number is :{Engine_No} and HLA_status not ok cam cap ok Status sent to plc")
                                                ...
                                                logger.debug(f"3cylinder Engine Number is :{Engine_No} S_PROCESS == 2 and CamCapstatus == NOT OK and HLA_status ==OK and CamCapstatus not ok Status sent to plc")
    ...
    def initCam(self):
        ...
        try:
            ...
            # Filter cameras by interface or other criteria
            device_list = [CAM1]  # Define CAM1 serial or identifier
            required_cams = self.filter_cameras(cam_list, device_list)

            if not required_cams:
                logger.error("No required cameras found.")
                return False

            # Run acquisition for the selected cameras
            if not self.acquire_images(required_cams):
                logger.error("Failed to acquire images from cameras.")
                return False
            ...
            logger.error("Error initializing cameras: %s", traceback.format_exc())
            return False
    ...
    def filter_cameras(self, cam_list, device_list):
        """Filter cameras based on the device serial numbers."""
        filtered_cams = []
        for cam in cam_list:
            serial_number = self.getSerialNumber(cam)
    ...
    def getSerialNumber(self, cam):
        try:
            ...
            logger.error("Error getting serial number: %s", traceback.format_exc())
        return None
...
def initializeLogger():
    ...
    try:
        ...
        # Create a TimedRotatingFileHandler for daily rotation
        log_file = "C:/Users/mahindra/insightzz/ALGORITHM_NEW_HLA/ALGORITHAM/LOG/hla_frame_capture.log"#configHashMap.get(CONFIG_KEY_NAME.LOG_FILE_PATH)+os.path.basename(__file__[:-2])+"log"
