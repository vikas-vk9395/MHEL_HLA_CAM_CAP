...
NOT_OK_CAP_SAVED_FOLDER_PATH = "C:/Users/mahindra/insightzz/ALGORITHM_NEW_HLA/ALGORITHAM/CAP_DEFECT_DATA/"
if DEMO_RUN_FLAG:
...
#     Engine_no = None
#     Engine_type = None
#     db_fetch = None  # Ensure variable is defined for `finally`

#     try:
#         # Connect to the database
#         db_fetch = pymysql.connect(host=DB_HOST, user=DB_USER, passwd=DB_PASS, db=DB_NAME)
#         with db_fetch.cursor() as cur:
#             # Fetch the latest engine number and type
#             query = "SELECT ENGINE_NUMBER, ENGINE_TYPE FROM engine_number_table ORDER BY ID DESC LIMIT 1;"
#             cur.execute(query)
#             data = cur.fetchone()

#             if data:
#                 Engine_no = str(data[0]) if data[0] else None
#                 Engine_type = str(data[1]) if data[1] else None

#     except Exception as e:
#         print(f"Database Error: {e}")
#         logger.error(f"getInferenceTrigger exception: {e}")
...
def getInferenceTrigger():
    ...
    for attempt in range(retries):
        db_fetch = None  # Ensure it's defined each loop
        try:
            db_fetch = pymysql.connect(host=DB_HOST, user=DB_USER, passwd=DB_PASS, db=DB_NAME)
            with db_fetch.cursor() as cur:
                query = "SELECT ENGINE_NUMBER, ENGINE_TYPE FROM engine_number_table ORDER BY ID DESC LIMIT 1;"
            ...
            logger.error(f"getInferenceTrigger attempt {attempt + 1} failed: {e}")
            time.sleep(delay)
    ...
    if type == 1:
        ...
        try:
            if position in status_mapping:
                for obj in OBJECT_LIST:
                    classname = obj[0]  # Access class name

                    # Set object detection flags based on class and position
                    ...
                    # Draw rectangle for unmatched classes
                        ...
                        # Put the classname text near the rectangle
                        font = cv2.FONT_HERSHEY_SIMPLEX
                        font_scale = 3
                        thickness = 5
                        text_position = (obj[1], obj[2] - 10)  # Position the text slightly above the rectangle

                        cv2.putText(original_image, classname, text_position, font, font_scale, color, thickness)             

                        # # Set all related flags to False for unmatched class names
                        # for key in ["I", "E", "Itwo", "Ithree", "Ifour", "Ifive", "Eone", "Etwo", "Ethree", "Efour"]:
                        #     if key.startswith(classname):
                        #         object_flags[key] = False
                        # Explicitly set relevant flags to False for unmatched class names
                        object_flags["I"] = False
                        object_flags["E"] = False
                        object_flags["Ione"] = False
                        object_flags["Itwo"] = False
                        object_flags["Ithree"] = False
                        object_flags["Ifour"] = False
                        object_flags["Eone"] = False
                        object_flags["Etwo"] = False
                        object_flags["Ethree"] = False
                        object_flags["Efour"] = False

                # Determine status based on object flags and position
                ...
                # Return the status for the given position
                return status[status_mapping[position]], original_image

        except Exception as e:
            print(f"Exception in HLA_CAM_CAP_INSPECTION_4CYLINDER CT: {e}")
            logger.debug(f"Exception in HLA_CAM_CAP_INSPECTION_4CYLINDER CT: {e}")

            # Return "NOT OK" in case of any error or if conditions aren't met
        ...
        if position in status_mapping:
            for obj in OBJECT_LIST:
                ...
                # Draw rectangle for unmatched classes
                    ...
                    print(f"classname for not ok is: {classname}")
                    color = (0, 0, 255)
                    cv2.rectangle(original_image, (obj[1], obj[2]), (obj[3], obj[4]), color, 3)
                    # Set all related flags to False
                    for key in ["I", "E", "Ione", "Itwo", "Ithree", "Eone", "Etwo", "Ethree"]:
                        if key.startswith(classname):
                            object_flags[key] = False
            ...
            # Return the updated status and the image
            return status[status_mapping[position]], original_image

    except Exception as e:
        print(f"Exception in HLA_CAM_CAP_INSPECTION_3CYLINDER: {e}")
        logger.debug(f"Exception in HLA_CAM_CAP_INSPECTION_3CYLINDER: {e}")
    ...
    try:
        if Position == "1":  # I2 position check
            ...
            for smalllabellist in OBJECT_LIST:
                ...
                if smalllabellist[2] < 630: # 500 # Left-side detection
                    ...
                    elif smalllabellist[0] == 'HLA_ROD_TOP' and ymin > 60:
            ...
            try:
                if '0' in hla_label_dict_left and '1' in hla_label_dict_left:
                    ...
                    thresholdPixel_LeftSide_1 = 51#41#36#25#14
        ...
        if Position == "2":  # I3 position check
            ...
            try:
                if '0' in hla_label_dict_left and '1' in hla_label_dict_left:
                    ...
                    thresholdPixel_LeftSide_2 = 147#140#130#110#101#95
            ...
            try:
                if '0' in hla_label_dict_right and '1' in hla_label_dict_right:
                    ...
                    thresholdPixel_RightSide_2 = 127#120#110#102#90#70
                    HlaSize = ymaxTR -yminTR
        ...
        if Position == "3":  # I4 position check
            ...
            try:
                if '0' in hla_label_dict_left and '1' in hla_label_dict_left:
                    ...
                    thresholdPixel_LeftSide_2 = 145#135#115#90#85
                    HlaSize = ymaxTL -yminTL
                    ymin_L_Value = yminTL -yminRTL
                    ymax_L_Value = ymaxTL -ymaxRTL
                    LeftSide_Cet = Lcy - cy 
        ...
        if Position == "4":  # I5 position check
            ...
            try:
                if '0' in hla_label_dict_left and '1' in hla_label_dict_left:
                    ...
                    thresholdPixel_LeftSide_2 = 135#120#110
                    HlaSize = ymaxTL -yminTL
                    ymin_L_Value = yminTL -yminRTL
                    ymax_L_Value = ymaxTL -ymaxRTL
        ...
        if Position == "5":  # E4 position check
            ...
            try:
                if '0' in hla_label_dict_left and '1' in hla_label_dict_left:
                    ...
                    hla_rod_top_L = hla_label_dict_left.get('1')
                    xminRTL, yminRTL, xmaxRTL, ymaxRTL,result, Lcx, Lcy = hla_rod_top_L[1:8]
                    thresholdPixel_LeftSide_1 =  55#45#37#27#22#10#5
                    thresholdPixel_LeftSide_2 = 140#134# 120#105#97
                    HlaSize = ymaxTL -yminTL
                    ymin_L_Value = yminTL -yminRTL
                    ymax_L_Value = ymaxTL -ymaxRTL
        ...
        if Position == "8":  # E1 position check
            ...
            for smalllabellist in OBJECT_LIST:
                ...
                if smalllabellist[2] < 590: #500 #630
                    if smalllabellist[0] == 'HLA_TOP' or smalllabellist[0] == "HLA_3CYL":
                        hla_label_dict_left['0'] = smalllabellist
                    elif smalllabellist[0] == 'HLA_ROD_TOP':
                        hla_label_dict_left['1'] = smalllabellist
            ...
            try:              
                if '0' in hla_label_dict_right and '1' in hla_label_dict_right:
                    ...
                    thresholdPixel_RightSide_2 = 63 #52#45#40#30
                    HlaSize = ymaxTR - yminTR
                    ymin_R_Value = yminTR -yminRTR
                    ymax_R_Value = ymaxTR -ymaxRTR
                    RightSide_Cet = cy -Rcy
    ...
    try:
        if Position == "1":  # I2 position check
            ...
            for smalllabellist in OBJECT_LIST:
                ...
                if smalllabellist[2] < 560:#630: #500 # Left-side detection
                    if smalllabellist[0] == 'HLA_TOP' or smalllabellist[0] == "HLA_3CYL":
                        hla_label_dict_left['0'] = smalllabellist
                    elif smalllabellist[0] == 'HLA_ROD_TOP' and ymin > 5: #Ymin -60
                        hla_label_dict_left['1'] = smalllabellist
            ...
            try:
                if '0' in hla_label_dict_left and '1' in hla_label_dict_left:
                    ...
                    thresholdPixel_LeftSide_2 = 50#47
                    LeftSide_Cet = Lcy - cy   
            ...
            try:              
                if '0' in hla_label_dict_right and '1' in hla_label_dict_right:
                    ...
                    thresholdPixel_RightSide_2 = 55#57
                    thresholdPixel_RightSide_1 = 67 #60
        ...
        if Position == "2":  # I3 position check
            ...
            try:
                if '0' in hla_label_dict_right and '1' in hla_label_dict_right:
                    ...
                    thresholdPixel_RightSide_1 = 87#81#75#57
                    thresholdPixel_RightSide_2 = 80#15#50
        ...
        if Position == "3":  # I4 position check
            ...
            try:
                if '0' in hla_label_dict_right and '1' in hla_label_dict_right:
                    ...
                    RightSide_Cord = xminRTR - xminTR
                    thresholdPixel_RightSide_1 = 0 #5
                    thresholdPixel_RightSide_2 = 80#70#65#60#55#50
        ...
        if Position == "4":  # I5 position check
            ...
            try:
                if '0' in hla_label_dict_left and '1' in hla_label_dict_left:
                    ...
                    thresholdPixel_LeftSide_2 = 50#30#12
        ...
        if Position == "5":  # E4 position check
            ...
            try:
                if '0' in hla_label_dict_right and '1' in hla_label_dict_right:
                    ...
                    thresholdPixel_RightSide_2 = 21#15#30
                    SizeOf_cy_line = cy_line -cy
        ...
        if Position == "7":  # E2 position check
            ...
            try:
                if '0' in hla_label_dict_right and '1' in hla_label_dict_right:
                    ...
                    thresholdPixel_RightSide_2 = 30#20
        ...
        if Position == "8":  # E1 position check
            ...
            try:
                if '0' in hla_label_dict_left and '1' in hla_label_dict_left:
                    ...
                    thresholdPixel_LeftSide_2 = 90 #30
        ...
        if Position == "1":  # I1 position check
            ...
            try:
                ...
                if '0' in hla_label_dict_left and '1' in hla_label_dict_left:
                    ...
                    thresholdPixel_LeftSide_1 = 10
                    thresholdPixel_LeftSide_2 = 70#60
                    HlaSize = ymaxTL - yminTL  #ok 232   #nok -185
                    # if ymaxTL > 545:
                    #     thresholdPixel_LeftSide_2 = 58

                    logger.debug(f"Left I1, HlaSize is {HlaSize}  ymaxTL: {ymaxTL}, ymaxRTL: {ymaxRTL}, xminTL: {xminTL}, "
                                 f"thresholdPixel_LeftSide_1: {thresholdPixel_LeftSide_1}, "
                                 f"thresholdPixel_LeftSide_2: {thresholdPixel_LeftSide_2}")
            ...
            try:
                ...
                if '0' in hla_label_dict_right and '1' in hla_label_dict_right:
                    ...
                    RightSide_Cord = xminRTR - xminTR
                    thresholdPixel_RightSide_1 = 73#58#48#40#30 
                    thresholdPixel_RightSide_2 = 10
                    HlaSize = ymaxTR - yminTR   #ok 230   #nok -184

                    logger.debug(f"Right I1, HlaSize is {HlaSize}  yminTR: {yminTR}, yminRTR: {yminRTR}, "
                                 f"thresholdPixel_RightSide_1: {thresholdPixel_RightSide_1}, "
                                 f"thresholdPixel_RightSide_2: {thresholdPixel_RightSide_2}")
        ...
        if Position == "2":  # I2 position check
            ...
            try:
                ...
                if '0' in hla_label_dict_left and '1' in hla_label_dict_left:
                    ...
                    xminRTL, yminRTL, xmaxRTL, ymaxRTL = hla_rod_top_L[1:5]
                    thresholdPixel_LeftSide_1 = 20
                    thresholdPixel_LeftSide_2 = 64#56#50 #20
        ...
        if Position == "3":  # I3 position check
            ...
            try:
                if '0' in hla_label_dict_left and '1' in hla_label_dict_left:
                    ...
                    hla_rod_top_L = hla_label_dict_left.get('1')
                    xminRTL, yminRTL, xmaxRTL, ymaxRTL = hla_rod_top_L[1:5]
                    thresholdPixel_LeftSide_1 = 100
                    thresholdPixel_LeftSide_2 = 60#40#30
            ...
            try:
                if '0' in hla_label_dict_right and '1' in hla_label_dict_right:
                  ...
                  #  Check_Line_distance_with_hla_ymin = yminTL_line - yminTL #ok = 101
                    ...
                    if yminTR < (yminRTR - thresholdPixel_RightSide_1) or yminTR > (yminRTR + thresholdPixel_RightSide_2) or Check_Line_distance_with_hla > 103 or Check_Line_distance_with_hla_ymin < 52 or Check_Line_distance_with_hla_ymin > 100 or SizeOf_cy_line > 15:   #or SizeOf_cy_line < 7 or SizeOf_cy_line > 18:   #ok-12
                        HLA_POSTION_RIGHT = "NOT OK"
                        logger.debug(f"HLA_POSTION_RIGHT = NOT OK")
                        color = (0, 0, 255)
                        cv2.rectangle(original_image, (xminTR, yminTR), (xmaxTR, ymaxTR), color, 12)
        ...
        if Position == "5":  # E2 position check
            ...
            try:
                ...
                if '0' in hla_label_dict_left and '1' in hla_label_dict_left:
                    ...
                    if ymaxTL < (ymaxRTL - thresholdPixel_LeftSide_1) or ymaxTL > (ymaxRTL + thresholdPixel_LeftSide_2) or Check_Line_distance_with_hla > 93 or Check_Line_distance_with_hla_ymin < 90 or SizeOf_cy_line > 30:   #ok-16
                        logger.debug(f"HLA_POSTION_LEFT = NOT OK")
                        HLA_POSTION_LEFT = "NOT OK"
                        color = (0, 0, 255)
                        cv2.rectangle(original_image, (xminTL, yminTL), (xmaxTL, ymaxTL), color, 12)
            ...
            try:
                if '0' in hla_label_dict_right and '1' in hla_label_dict_right:
                    ...
                    if yminTR < (yminRTR - thresholdPixel_RightSide_1) or yminTR > (yminRTR + thresholdPixel_RightSide_2) or Check_Line_distance_with_hla > 102 or Check_Line_distance_with_hla_ymin < 70 or SizeOf_cy_line > 15:
                        HLA_POSTION_RIGHT = "NOT OK"
                        logger.debug(f"HLA_POSTION_RIGHT = NOT OK")
                        color = (0, 0, 255)
                        cv2.rectangle(original_image, (xminTR, yminTR), (xmaxTR, ymaxTR), color, 12)
...
def run(maskRCNNObj):
    ...
    try:
        while True:
            ...
            #3_Cylinder = 1 = I2, 2 = I3, 3 = I4, 4 = E3, 5 = E2, 6 = E1  Human_Detection  FRAME_CAPTURE
                            ...
                            if len(img_list) == 6:
                                ...
                                for img_name in img_list:
                                    ...
                                    if OBJECT_LIST == [] or OBJECT_LIST is None:
                                        ...
                                        if position == "4":
                                            CAM_CAP_NUMBER_STATUS_POS4,original_image = HLA_CAM_CAP_INSPECTION_3CYLINDER(position,OBJECT_LIST,original_image)
                                            if CAM_CAP_NUMBER_STATUS_POS4 == "NOT_OK" or CAM_CAP_NUMBER_STATUS_POS4 == "NOT OK":
                                                if os.path.exists(os.path.join(NOT_OK_CAP_SAVED_FOLDER_PATH, TodaysDate)) is False: 
                                                    os.mkdir(os.path.join(NOT_OK_CAP_SAVED_FOLDER_PATH, TodaysDate))
                                                if os.path.exists(os.path.join(os.path.join(NOT_OK_CAP_SAVED_FOLDER_PATH, TodaysDate),Engine_no)) is False:
                                            ...
                                            if HLA_PostionCheck_pos5 == "OK" and CAM_CAP_NUMBER_STATUS_POS4 == "OK" and class_check_cam1 is True:
                                                pos = (50, 60)
                                               # putText_notok_ok("CamCap is OK",original_image, pos)
                                                POS_6_E3_STATUS = "OK"
                                            else:
                                        ...
                                        if position == "5":
                                           ...
                                           # CAM_CAP_NUMBER_STATUS_POS5 = "OK"
                                            ...
                                            if HLA_PostionCheck_pos6 == "OK" and CAM_CAP_NUMBER_STATUS_POS5 == "OK" and class_check_cam1 is True:
                                                pos = (50, 60)
                                               # putText_notok_ok("CamCap is OK",original_image, pos)
                                                POS_7_E2_STATUS = "OK"
                                            else:
                                        ...
                                        if position == "6":
                                           ...
                                           # CAM_CAP_NUMBER_STATUS_POS6 = "OK"
                                            ...
                                            if HLA_PostionCheck_pos7 == "OK" and CAM_CAP_NUMBER_STATUS_POS6 == "OK" and class_check_cam1 is True:
                                                pos = (50, 60)
                                                #putText_notok_ok("CamCap is OK",original_image, pos)
                                                POS_8_E1_STATUS = "OK"
                                            else:
                                ...
                                if CAM_CAP_NUMBER_STATUS_POS1 == "OK" and CAM_CAP_NUMBER_STATUS_POS2 == "OK" and CAM_CAP_NUMBER_STATUS_POS3 == "OK" and CAM_CAP_NUMBER_STATUS_POS4 == "OK" and CAM_CAP_NUMBER_STATUS_POS5 == "OK" and CAM_CAP_NUMBER_STATUS_POS6 == "OK":
                                    OVERALL_CAM_CAP_RESULT = "OK"
